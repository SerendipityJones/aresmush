    ...
    class ChannelRecallCmd
      ...
      def handle
        Channels.with_an_enabled_channel(self.name, client, enactor) do |channel|
          ...
          list = messages.map { |m| " [#{OOCTime.local_long_timestr(enactor, m.created_at)}] #{Channels.display_name(enactor, channel)}#{m.message}".sub(/(\w+) <span class='handle'>(\(@\w+\))<\/span>/, "\\1 \\2")}
